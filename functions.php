...
{
    global $db;
    $rows = $db->get_results("SELECT id_wisata, nama_wisata FROM wisata_tb ORDER BY id_wisata");
    foreach ($rows as $row) {
        if ($row->id_wisata == $selected)
            $a .= "<option value='$row->id_wisata' selected>$row->nama_wisata</option>";
        else
            $a .= "<option value='$row->id_wisata'>$row->nama_wisata</option>";
    }
    return $a;
}
...
{
    global $db;
    $row = $db->get_row("SELECT gambar FROM wisata_tb WHERE id_wisata='$ID'");
    if ($row) {
...
{
    global $db;
    $row = $db->get_row("SELECT gambar FROM galeri_tb WHERE id_galeri='$ID'");
    if ($row) {
